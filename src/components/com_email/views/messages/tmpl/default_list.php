...
function confirm_empty_deleted_items() {
	...
	if (answer){
		window.location = "index.php?option=com_email&task=empty_deleted_items&folder=<?php echo text::_($this->folder, true); ?>";
	}
}
...
function confirm_delete_folder(mailbox, label) {
	var remove_link = "index.php?option=com_email&task=delete_mailbox&folder=<?php echo text::_($this->folder, true); ?>";
...
<div id="right_col">
	...
	<div style="float:right;" class="new">
		...
		<?php echo text::_(_LANG_EMAIL_NEW_FOLDER); ?>
		</a>
	</div>
	...
	<ul>
		<?php if (is_array($this->boxes) && count($this->boxes) > 0) : ?>
			...
			<li class="container_<?php echo $key; ?>">
				...
				<img border="0" src="templates/<?php echo $this->config->template; ?>/images/icons/generic/16x16/edit.png" alt="<?php echo _LANG_RENAME; ?>" />
				 <?php echo _LANG_RENAME; ?>
				</a>
				</li>
				
				<li>
				<a href="Javascript:confirm_delete_folder('<?php echo text::_($box['nameX'], true); ?>', '<?php echo substr($box['nameX'], 0, 32); if (strlen($box['nameX']) > 33) { echo '...'; }; ?>');" title="<?php echo _LANG_DELETE; ?>">
				<img border="0" src="templates/<?php echo $this->config->template; ?>/images/icons/email/16x16/mail_delete-16x16.png" alt="<?php echo _LANG_DELETE; ?>" />
			...
			<?php endforeach;?>
		<?php else : ?>
		<?php echo text::_( _LANG_NO_MAILBOXES ); ?>
		<?php endif; ?>
...
<div id="email_detail_actions">
	<div>
		<a href="<?php echo route::_("index.php?option=com_email&amp;view=messages&amp;layout=form"); ?>" title="<?php echo text::_( _LANG_NEW ); ?>">
		<img border="0" src="templates/<?php echo $this->config->template; ?>/images/icons/generic/32x32/new.png" alt="<?php echo text::_( _LANG_NEW ); ?>" />
		</a>
	</div>
	<div>
		<a class="modal" href="index.php?option=com_email&amp;view=messages&amp;folder=<?php echo text::_($this->folder, true); ?>&amp;layout=move_email&tmpl=component" rel="{handler: 'iframe', size: {x: 400, y: 160}}" title="<?php echo _LANG_EMAIL_MOVE_TO_FOLDER; ?>">
		<img border="0" src="templates/<?php echo $this->config->template; ?>/images/icons/email/32x32/move_email.png" alt="<?php echo _LANG_EMAIL_MOVE_TO_FOLDER; ?>" />
		</a>
	</div>
	<div>
		<a href="Javascript:confirm_delete(1);" title="<?php echo _LANG_EMAIL_MOVE_TO_TRASH; ?>">
		<img border="0" src="templates/<?php echo $this->config->template; ?>/images/icons/email/32x32/move_to_trash.png" alt="<?php echo _LANG_EMAIL_MOVE_TO_TRASH; ?>" />
		</a>
	</div>
	<div>
		<a href="Javascript:confirm_empty_trash();" title="<?php echo _LANG_EMAIL_EMPTY_TRASH; ?>">
		<img border="0" src="templates/<?php echo $this->config->template; ?>/images/icons/generic/32x32/empty_trash.png" alt="<?php echo _LANG_EMAIL_EMPTY_TRASH; ?>" />
		</a>
	</div>
	<!-- 
	<div>
		<a href="Javascript:confirm_delete();" title="<?php echo _LANG_EMAIL_FLAG_DELETED; ?>">
		<img border="0" src="templates/<?php echo $this->config->template; ?>/images/icons/generic/32x32/remove.png" alt="<?php echo _LANG_EMAIL_FLAG_DELETED; ?>" />
		</a>
	</div>
	<div>
...
<div class="ioffice_table">
	...
	<?php 
  	if ($email->answered == 1) { 
  		$status_icon = '<img src="templates/'.$this->config->template.'/images/icons/email/replied.png" alt="Unread" />';
  	}
  	elseif ($email->seen == 0) {
  		$status_icon = '<img src="templates/'.$this->config->template.'/images/icons/email/new.png" alt="Unread" />';
  	}
  	else { 
  		$status_icon = '<img src="templates/'.$this->config->template.'/images/icons/email/read.png" alt="Read" />'; 
  	}
  	?>
  	<div onclick="select_row(this, <?php echo $email->uid; ?>);" class="ioffice_row<?php echo $k; ?> seen<?php echo $email->seen; ?> <?php if ($email->deleted == 1) echo 'deleted'; ?> container_<?php echo $email->uid; ?>">
    	...
    	<div class="ioffice_cell <?php if ($email->deleted == 1) echo 'undelete'; ?>" style="text-decoration: none;">
			...
			<img src="templates/<?php echo $this->config->template; ?>/images/icons/email/16x16/mail_reply-16x16.png" alt="<?php echo _LANG_EMAIL_REPLY; ?>" />
			...
			<img src="templates/<?php echo $this->config->template; ?>/images/icons/email/16x16/mail_forward-16x16.png" alt="<?php echo _LANG_FORWARD; ?>" />
			...
			<img src="templates/<?php echo $this->config->template; ?>/images/icons/email/16x16/mail_delete-16x16.png" alt="<?php echo _LANG_DELETE; ?>" />
			 <?php echo _LANG_DELETE; ?>
			</a>
			</li>
	  ...
	  <td align="left">
		  ...
		  if ($this->messages['pages'] > 1) {
			...
			for ($i=0; $i<$this->messages['pages']; $i++) {
			  ...
			  if ($this->messages['current_page'] != ($i+1)) {
			    echo '<a href="'.route::_('index.php?option=com_email&view=messages&folder='.$this->folder.'&order_by='.$this->messages['sorting']['by'].'&order_direction='.$this->messages['sorting']['direction'].'&page='.($i+1)).'&per_page='.$this->messages['per_page'].'">';
			    echo ($i+1);
			    echo '</a>';
			  }
			  else {
		...
		<form name="form1">
		  Messages per page: <select name="menu1" onChange="window.location = this.options[selectedIndex].value;">
			...
			<option value="<?php echo route::_('index.php?option=com_email&view=messages&folder='.$this->folder.'&order_by='.$this->messages['sorting']['by'].'&order_direction='.$this->messages['sorting']['direction'].'&per_page=25'); ?>" <?php if ($this->messages['per_page'] == '25') { echo 'selected'; } ?>>25</option>
			<option value="<?php echo route::_('index.php?option=com_email&view=messages&folder='.$this->folder.'&order_by='.$this->messages['sorting']['by'].'&order_direction='.$this->messages['sorting']['direction'].'&per_page=50'); ?>" <?php if ($this->messages['per_page'] == '50') { echo 'selected'; } ?>>50</option>
			<option value="<?php echo route::_('index.php?option=com_email&view=messages&folder='.$this->folder.'&order_by='.$this->messages['sorting']['by'].'&order_direction='.$this->messages['sorting']['direction'].'&per_page=100'); ?>" <?php if ($this->messages['per_page'] == '100') { echo 'selected'; } ?>>100</option>
		  </select>
		</form>
